...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="bg-white/95 border-b border-[#E3E8E5] sticky top-0 z-50 backdrop-blur-xl">
        <div className="w-full px-4">
          <div className="flex items-center justify-between gap-6 h-14">
            ...
            <div className="flex items-center gap-4 flex-shrink-0">
              ...
              <Link to="/cart" className="relative p-1.5 text-[#4B534D] hover:text-[#10B982] transition" aria-label="Cart">
                <FiShoppingCart size={18} />
                {cartCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 bg-[#10B982] text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center font-medium">
                    {cartCount}
                  </span>
                )}
              </Link>
              ...
              <div className="relative">
                <button onClick={() => setAccountOpen(!accountOpen)} className="p-1.5 text-[#4B534D] hover:text-[#10B982] transition" aria-label="Account">
                  <FiUser size={18} />
                </button>
                {accountOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-[#E3E8E5] rounded-lg shadow-xl z-50" onMouseLeave={() => setAccountOpen(false)}>
                    {isAuthenticated ? (
                      <>
                        <Link to="/profile" onClick={() => setAccountOpen(false)} className="block px-4 py-2.5 text-sm text-[#3A423E] hover:bg-[#F5FAF7] hover:text-[#10B982] transition">My Profile</Link>
                        <Link to="/orders" onClick={() => setAccountOpen(false)} className="block px-4 py-2.5 text-sm text-[#3A423E] hover:bg-[#F5FAF7] hover:text-[#10B982] transition">My Orders</Link>
                        <Link to="/wishlist" onClick={() => setAccountOpen(false)} className="block px-4 py-2.5 text-sm text-[#3A423E] hover:bg-[#F5FAF7] hover:text-[#10B982] transition">Wishlist</Link>
                        {user?.roles?.includes('ADMIN') && (
                          <Link to="/admin" onClick={() => setAccountOpen(false)} className="block px-4 py-2.5 text-sm text-[#3A423E] hover:bg-[#F5FAF7] hover:text-[#10B982] transition">Admin Panel</Link>
                        )}
                        <button onClick={() => { logout(); navigate('/'); setAccountOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm text-[#3A423E] hover:bg-[#F5FAF7] border-t border-[#EEF3F0] transition">Sign Out</button>
                      </>
                    ) : (
                      <>
                        <Link to="/login" onClick={() => setAccountOpen(false)} className="block px-4 py-2.5 text-sm text-[#3A423E] hover:bg-[#F5FAF7] hover:text-[#10B982] transition">Sign In</Link>
                        <Link to="/register" onClick={() => setAccountOpen(false)} className="block px-4 py-2.5 text-sm text-[#3A423E] hover:bg-[#F5FAF7] hover:text-[#10B982] transition">Create Account</Link>
                      </>
                    )}
                  </div>
                )}
            ...
            <button className="md:hidden p-1.5 text-[#4B534D] hover:text-[#10B982] transition" onClick={() => setOpen(!open)} aria-label="Menu">
