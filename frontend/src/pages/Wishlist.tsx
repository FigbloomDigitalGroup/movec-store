...
export default function WishlistPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-2 text-white">Wishlist</h1>
      {!isAuthenticated && (
        <p className="text-gray-300 mb-6 text-sm">Browsing as guest. Items are saved in your browser.</p>
      )}
          ...
          {items.map((item: any) => (
            <div key={item.productId || item.id} className="bg-white/80 backdrop-blur-sm rounded-xl shadow p-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                <Link to={`/products/${item.slug}`} className="font-semibold hover:text-blue-600">{item.name}</Link>
